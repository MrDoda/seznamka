 ...
 * @package Nette\Forms
 */
class NRules extends NObject implements IteratorAggregate
{
	/** @internal */
	...
	private function adjustOperation($rule)
	{
		...
		if (!$this->getCallback($rule)->isCallable()) {
			...
			throw new InvalidArgumentException("Unknown operation$operation for control '{$rule->control->getName()}'.");
		}
	}
