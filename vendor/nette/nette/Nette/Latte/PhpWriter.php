...
class NPhpWriter extends NObject
{
	...
	public function write($mask)
	{
		...
		$me = $this;
		$mask = NStrings::replace($mask, '#%escape(\(([^()]*+|(?1))+\))#', create_function('$m', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('me'=>$me)).'-1], EXTR_REFS);
		...
		create_function('$m', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('me'=>$me,'word'=> $word, 'args'=>& $args)).'-1], EXTR_REFS);
	...
	public function escape($s)
	{
		switch ($this->compiler->getContentType()) {
			...
			case NLatteCompiler::CONTENT_XML:
				$context = $this->compiler->getContext();
				if ($context[0] === NLatteCompiler::CONTEXT_COMMENT) {
					return "NTemplateHelpers::escapeHtmlComment($s)";
				}
